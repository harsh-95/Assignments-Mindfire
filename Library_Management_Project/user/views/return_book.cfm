...
<html>
	...
	<body>
		...
		<div class="container" style="background-image: url('../../assets/images/books.jpg');">
			...
			<cfquery name="fetch" datasource="Library">
				...
				FROM Books
				WHERE Books.ISBN IN (SELECT ISBN FROM Books_Record WHERE Email_Id = '#SESSION.login_id#' AND Return_Date_Time IS NULL)
			</cfquery>
	...
	<!---			<td><button class="btn-edit" id="issue" >Issue</button></td>   --->
					<td><a class="btn-issue" id="issue" href="../handlers/issue_book_handler.cfm?id=return&isbn=#ISBN#">RETURN</a></td>
				</tr>
			</cfoutput>
        </tbody>
    </table>
