 ...
 $('#add-book').click(function(e) {
 		...
 		var submitForm = areAllFieldsValid();
 			...
 			if(submitForm){
 				...
 				$.ajax({
 				      type: "POST",
 				      url: "../components/manage_books.cfc?method=fetchBookByIsbn",
 				      data: {isbnn: isbn1},
 				      success: function(res){

				    	  res = JSON.parse(res);
				    	  		// if book record is not found
			          			...
			          			//book record is found 
			          				...
			          				// if the book was deleted(true)
			          				if((res.DATA[0][6])){
			          					// update the details
			          					addTheBook('update');
			          				}
			          				// if the book wasn't deleted(false)
...
$('#request-book').click(function(e) {
		...
		for(i = 0 ; i < validateArray.length ; i++){
			...
			if(submitForm){
				...
				$.ajax({
				      type: "POST",
				      url: "../../admin/components/manage_books.cfc?method=fetchBookByIsbn",
				      data: {isbnn: isbn1},
				      success: function(res){
...
function requestTheBook(){
	 ...
	 isbn1 = $("#isbn").val();
		...
		$.ajax({
	      type: "POST",
	      url: "../../user/components/book_options.cfc?method=requestBook",
